...
!=----------------------------------------------------------------------------------=!
   ...
   SUBROUTINE runcp_uspp_x &
      ...
      !SHERIF
      use electrons_base,      only : nbnd,nbsp,f,nupdwn,nel
      USE input_parameters, ONLY: emass_preconditioning,hmass,emass,enable_hole_mass
      USE input_parameters, ONLY: emptymass
      !END SHERIF
      !
      IMPLICIT NONE
      !
...
!dir$ attributes align: 4096 :: emadt2, emaver, c2, c3, c2tmp, c3tmp, tg_rhos, ftmp, itmp
   ...
   !SHERIF end
     ...
     ccc    = fccc * dt2bye
     !SHERIF: for hmass, apply same cuttoff as emass
...
!     IF( lwf ) THEN
     ...
     ELSE
        ...
        END IF
      !SHERIF
      !write(*,*) 'SHERIF>>>hole mass:',emass_preconditioning,hmass,emass
        !DO i = 1, nbnd
         
            !write(*,*) 'Band occupation in group:',i,'of',nbsp_bgrp,nbnd,nbsp,nupdwn,'=',f(i*2)
      ...
      !END SHERIF
        DO i = 1, nbsp_bgrp, incr
           ...
           !SHERIF
               !write(*,*) 'Band occupation in group:',i,'of',nbsp_bgrp,nbnd,nbsp,incr,'=',f_bgrp(i),nel(1),nel(2)
           !END SHERIF
           IF( dffts%has_task_groups ) THEN
              !
           ...
           DO idx = 1, incr, 2
              IF( i + idx - 1 <= nbsp_bgrp ) THEN
                 IF (tsde) THEN
                    CALL wave_steepest( cm_bgrp(:, i+idx-1 ), c0_bgrp(:, i+idx-1 ),&
                           mass_coeff*emaver, c2(:), ngw, idx_in )
                    CALL wave_steepest( cm_bgrp(:, i+idx   ), c0_bgrp(:, i+idx   ),&
                           mass_coeff*emaver, c3(:), ngw, idx_in )
                 ELSE
                     !SHERIF begin
                     !Multiply every emaver by mass_coeff
                     mass_coeff = 1
                     if( enable_hole_mass ) then
                        band_shift = 0
                        if( i+idx-1.gt.(nbsp_bgrp+1)/2) band_shift = (nbsp_bgrp+1)/2
                        if ( f(i+idx-1).lt.1 .and.  f(i+idx-1).gt.0 .and. i+idx-1.le.band_shift+nel(1)) then
                           mass_coeff = emass / hmass !basically replaces emass with hmass in emaver
                        else if ( f(i+idx-1).lt.1 .and.  f(i+idx-1).gt.0 .and. i+idx-1.gt.band_shift+nel(1)) then
                           mass_coeff = emass / emptymass !basically replaces emass with emptymass in emaver
                        end if
                        !write(*,*) 'SHERIF>>>',i+idx-1,'occupation is ',f(i+idx-1),nel(1),mass_coeff,idx,band_shift
                     end if
                     !SHERIF end
                    CALL wave_verlet( cm_bgrp(:, i+idx-1 ), c0_bgrp(:, i+idx-1 ), verl1, verl2,& 
                           mass_coeff*emaver, c2(:), ngw, idx_in )
                     !SHERIF begin
                     !Multiply every emaver by mass_coeff
                     mass_coeff = 1
                     if( enable_hole_mass ) then
                        band_shift = 0
                        if( i+idx.gt.(nbsp_bgrp+1)/2) band_shift = (nbsp_bgrp+1)/2
                        if ( f(i+idx).lt.1 .and.  f(i+idx).gt.0.and. i+idx.le.band_shift+nel(1)) then
                           mass_coeff = emass / hmass !basically replaces emass with hmass in emaver
                        else if ( f(i+idx).lt.1 .and.  f(i+idx).gt.0 .and. i+idx.gt.band_shift+nel(1)) then
                           mass_coeff = emass / emptymass !basically replaces emass with emptymass in emaver
                        end if
                        !write(*,*) 'SHERIF>>>',i+idx,'occupation is ',f(i+idx), nel(2),mass_coeff,idx
                     end if
                     !SHERIF end
                    CALL wave_verlet( cm_bgrp(:, i+idx   ), c0_bgrp(:, i+idx   ), verl1, verl2,&
                           mass_coeff*emaver, c3(:), ngw, idx_in )
                 ENDIF
                 IF ( gstart == 2 ) THEN
